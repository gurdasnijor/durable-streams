...
interface StreamRecord {
  readonly path: Store.StreamPath
...
const offsetOf = (n: number): Store.Offset => String(n)
...
const producerKey = (input: Store.AppendInput): Option.Option<string> =>
  Option.map(input.producer, (p) => `producer:${p.id}`)
...
const decide = (
...
): { decision: Store.AppendDecision; next: Option.Option<StreamRecord> } => {
  ...
  // --- Producer rules ------------------------------------------------------
  if (Option.isSome(input.producer)) {
    const p = input.producer.value
    const ps = record.producers.get(p.id)
    ...
    const newBytes = concatBytes(record.bytes, input.body)
...
const makeStore = (state: MemoryState): Store.StoreShape => {
  ...
  ): Effect.Effect<Store.CreateDecision, ProtocolError.ProtocolError> =>
    STM.commit(
      STM.gen(function* () {
  ...
  const append = (
    input: Store.AppendInput
  ): Effect.Effect<Store.AppendResult, ProtocolError.ProtocolError> =>
    STM.commit(
      STM.gen(function* () {
        const existing = yield* TMap.get(state.streams, input.path)
  ...
  const read = (
    ...
    offset: Store.Offset
  ): Effect.Effect<Store.ReadChunk, ProtocolError.ProtocolError> =>
    STM.commit(
      STM.gen(function* () {
  ...
  const head = (
    path: Store.StreamPath
  ): Effect.Effect<Store.StreamTail, ProtocolError.ProtocolError> =>
    STM.commit(
      STM.gen(function* () {
        const existing = yield* TMap.get(state.streams, path)
        if (Option.isNone(existing)) {
