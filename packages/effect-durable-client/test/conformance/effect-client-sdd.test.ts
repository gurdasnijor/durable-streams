...
import { FetchHttpClient, HttpApi } from "@effect/platform"
...
describe(`effect-client package and HttpApi conformance`, () => {
  it(`effect-client.PACKAGE.1 effect-client.PACKAGE.3 effect-client.PACKAGE.4 effect-client.PACKAGE.5 wires the renamed package to shared protocol only`, async () => {
    ...
    expect(pkg.dependencies).toHaveProperty(`durable-streams-protocol`)
  ...
  it(`effect-client.PACKAGE.2 effect-client.CONFORMANCE.1 effect-client.CONFORMANCE.4 preserves conformance over source compatibility`, async () => {
    ...
    expect(exports.exports).toHaveProperty(`./ApiClient`)
  })

  it(`effect-client.HTTP_API.1 effect-client.HTTP_API.2 effect-client.HTTP_API.3 effect-client.HTTP_API.4 lowers generated control errors`, () => {
    const lowered = lowerControlError(
      new ProtocolConfigConflict({
        code: `CONFIG_CONFLICT`,
        message: `different normalized config`,
      })
    )

    expect(lowered._tag).toBe(`DurableStream/ConfigConflict`)
  })

  it(`effect-client.HTTP_API.1 effect-client.HTTP_API.5 preserves nested stream paths outside deleteStream route segments`, () => {
    let reflected:
      | {
          readonly path: string
          readonly encodedUrlParams: unknown
        }
      | undefined

    HttpApi.reflect(DurableStreamsApi, {
      onGroup: () => {},
      onEndpoint: ({ group, endpoint }) => {
        if (
          group.identifier === `subscriptions` &&
          endpoint.name === `deleteStream`
        ) {
          if (Option.isNone(endpoint.urlParamsSchema)) {
            throw new Error(`deleteStream missing urlParams schema`)
          }
          reflected = {
            path: endpoint.path,
            encodedUrlParams: Schema.encodeUnknownSync(
              endpoint.urlParamsSchema.value
            )({ path: `rooms/general/messages` }),
          }
        }
      },
    })
    ...
    expect(reflected).toEqual({
      path: `/__ds/subscriptions/:id/streams`,
      encodedUrlParams: { path: `rooms/general/messages` },
    })
  })
