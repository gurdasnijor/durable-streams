...
const make = (
  ...
  const makeReadonlyHandle = <A, I>(
    ...
    const readStream = (opts: ReadHandleOptions | undefined) => {
      ...
      if (opts?.from === `now`) {
        return Stream.unwrap(
          Reader.head(endpoint).pipe(Effect.map((h) => makeRead(h.offset)))
        )
      }
      return makeRead(offsetFrom(opts?.from))
    }
    const controlStream = (opts: ReadHandleOptions | undefined) => {
      ...
      if (opts?.from === `now`) {
        return Stream.unwrap(
          Reader.head(endpoint).pipe(Effect.map((h) => makeControl(h.offset)))
        )
      }
      return makeControl(offsetFrom(opts?.from))
    }
    return {
      path: streamPath,
